...
起動時に自動生成されます。  
ステップ数の初期設定は16です。  
ステップシーケンサーは3oct（最大値は共通設定モードで変更可能）の間で生成されます。
オクターブの加算設定は2番目（±0位置）です。  
...
| **MODE長押し+ロータリーエンコーダー** | ステップシーケンサーのオクターブの加算（-1～+2の4段階） |
...
横の並びはOUT1,2,3,4,5,6への割り当て枠で、1～8ビットが割り当てられます。  
縦の並びはパターンバリエーションです。  
初期設定は0（OUT1=1bit, OUT2=2bit, OUT3=3bit, OUT4=4bit, OUT5=5bit, OUT6=6bit）です。  
...
- **ステップシーケンサ設定モード**
  ...
  - **ステップシーケンサーのオクターブ設定**  
    レインボーカラーでステップシーケンサーのオクターブ設定を-1～+2の4段階表示
...
- **ステップシーケンサーのオクターブ最大値設定**
  ステップシーケンサーの再生成時のオクターブ最大値を変更できます。
  初期設定は3です。

- **ステップシーケンサーへ入力するクロックの分割数設定**
  クロック入力に対してステップシーケンサーのCV変化を半分以下に設定変更できます。  
  初期設定は/1（クロックそのまま）
...
※RP2040 ZEROのGPIO出力で使用している3V3の出力電圧値に依存するため、RP2040 ZERO交換時にCV出力電圧を再調整する必要が生じる可能性があります。  
※出力電圧・出力電流、消費電流については出力へ接続した負荷により変動する可能性があります。  
...
1. モジュールのUSB端子をPCに接続します。
2. BOOTSELボタンを押しながら電源を入れると、USBドライブとして認識されます。
3. 最新のファームウェア（.uf2ファイル）をドラッグ＆ドロップしてください。
4. 書き込み後、自動的に再起動します。

### 8.2-CV出力電圧調整方法

1. MODEボタンを押しながら電源を入れます。
2. メンテナンスモードで立ち上がります（電源投入時LEDがピンク色になります）
3. エンコーダーを操作すると12半音単位でCVが可変し、1オクターブごとにLEDがピンクになります。
4. 60クリック分回して最大電圧（最高音）にします。
5. CV出力に電圧計を接続します。
6. CVトリムポット（パネル中央付近の金色のマイナスネジ）を回して5000mVになるように調整します。
7. エンコーダー押し込みでメンテナンスモードを抜けます。
